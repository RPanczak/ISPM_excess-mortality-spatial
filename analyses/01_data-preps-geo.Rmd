...
```{r include=FALSE}
# st_gg written alter after processing
...
st_se21 <- 
  st_difference(st_se21, st_union(islands))
...
# st_write(islands, "data/swisstopo/st_se21.gpkg", layer = "islands", append = TRUE)

write_rds(st_se21, "data/swisstopo/st_se21.Rds")
```

```{r include=FALSE}
st_se21 <- read_rds("data/swisstopo/st_se21.Rds") #%>% filter(as.numeric(area) > 10000)
```

### Combined 

Lakes dataset ~~(selecting features more than 10km^2 in size)~~ was then used to erase area of municipalities that extends over lakes. Issue is described in [#48](https://github.com/RPanczak/ISPM_geo-mortality/issues/48).  

```{r}
st_gg21 <- 
  st_difference(st_gg21, st_union(st_se21)) %>% 
...
swisstopo borders (in blue below) are larger in size (and slower to plot) but offer better resolution and will be used for certain spatial operations (like linking to SEP) whereas BfS data are smaller in size and easier to plot. Example of municipality Bern:  
...
<!-- Additionally datasets differ in terms of conceptualization of the boundaries of water bodies. BfS data reach the shore of the lake, whereas swisstopo extends them over lakes. Example of municipality *Delley-Portalban* by the *Lac de Neuchâtel*:   -->
...
BfS stores data as `MULTIPOLYGON`. In such case municipality split over several disconnected areas is stored as *single record*. Example of municipality Monthey which consists of four separate areas:  
...
The same municipality in swisstopo data is stored as `POLYGON` type. In such case it will be represented as *four separate records* (four individual polygons) instead of one.  
...
ar_21_cor <- st_read("data/geo/ar_21_cor.shp") %>% 
  select(-ARNAME) %>% 
...
write_rds(argr_21_cor, "data/geo/argr_21_cor.Rds")
write_rds(ar_21_cor, "data/geo/ar_21_cor.Rds")
```

```{r eval=FALSE, include=FALSE}
# there is some unresolved drama here
ar_21_cor %>% 
...
```{r eval=FALSE, include=FALSE}
sp <- maptools::readShapePoly("data/geo/ar_21_cor.shp")
...
```{r eval=FALSE, include=FALSE}
unlink("data/geo/argr_21.*")
unlink("data/geo/ar_21.*")
```
...
write_rds(st_argr_21, "data/geo/st_argr_21.Rds")
write_rds(st_ar_21, "data/geo/st_ar_21.Rds")
```

```{r eval=FALSE, include=FALSE}
sp <- maptools::readShapePoly("data/geo/st_ar_21.shp")
...
tm_shape(se21, is.master = FALSE) +
  ...
  tm_shape(ar_21_cor, is.master = TRUE) +
  tm_borders(col = "darkgray", lwd = 1, alpha = 0.5) +
  tm_shape(argr_21_cor) +
...
tm_shape(se21, is.master = FALSE) +
  tm_fill(col = "lightskyblue", alpha = 0.5) +
  tm_shape(ar_21_cor %>% mutate(border = factor(border))) +
...
```{r}
tg3o <- 
  st_read("data-raw/BfS/tg3o/K5tg3ovf_ch2001Poly.shp",
          as_tibble = TRUE) %>% 
  st_set_crs(21781) %>% 
  st_union() %>% 
  st_transform(2056)
...
TODO
would work for viz purposes
if spatial analytical work done on it - needs solution to clean small polygon slivers

```{r}
test <- st_intersection(st_gg21, tg3o)

frq(st_is_valid(ar_21_cor))

test <- st_intersection(ar_21_cor, tg3o)

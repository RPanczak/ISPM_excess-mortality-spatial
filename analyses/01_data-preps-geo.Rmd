...
```{r include=FALSE}
unique(st_geometry_type(gg22))

# gg22 written after after processing
write_rds(kt22, "data/BfS/kt22.Rds")
write_rds(se22, "data/BfS/se22.Rds")

st_write(kt22, "data/geo/BfS.gpkg", 
         layer = "kt22", append = FALSE)
st_write(se22, "data/geo/BfS.gpkg", 
         layer = "se22", append = FALSE)
```
...
2022 boundaries are needed to link to voting patterns.  
...
Freienbach & Rapperswil-Jona are disconnected on the map of boundaries but connected IRL:  

![No connectivity](../docs/conn4.png)
On top of that shape of municipality consists of multiple features:
...
### Region with twelve neighbour (max case)
...
tg3o_wm_q <- spdep::edit.nb(tg3o_wm_q, polys = as(tg3o, "Spatial"))

readr::write_rds(tg3o_wm_q, "data/nb/tg3o_wm_q_cor.Rds")
```

```{r include=FALSE}
tg3o_wm_q <- read_rds("data/nb/tg3o_wm_q_cor.Rds")
...
plot(as_Spatial(tg3o), borders = "lightgrey") 
plot(tg3o_wm_q, coordinates(as_Spatial(tg3o)), 
     pch = 19, cex = 0.6, add = TRUE, col = "red")
```

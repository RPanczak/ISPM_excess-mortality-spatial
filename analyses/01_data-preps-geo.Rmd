...
st_write(kt22, "data/geo/BfS.gpkg", 
         layer = "kt22", append = TRUE)
st_write(se22, "data/geo/BfS.gpkg", 
         layer = "se22", append = TRUE)
```
...
Dataset now consists of `r scales::number(length(unique(tg3o$GMDNR)), big.mark = ",")` communities.
...
tg3o_ar %>% 
  st_drop_geometry() %>% 
  select(ARNR) %>% 
  anti_join(ar_21 %>% st_drop_geometry())
```

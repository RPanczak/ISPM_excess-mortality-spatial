...
deaths_1969_2020 <- read_fst("data-raw/BfS/deaths_1969_2020_raw.fst") %>% 
  ...
  group_by(kanton_bezirk_gemeinde, jahr, staatsangehorigkeit_kategorie, altersklasse, geschlecht) %>% 
...
geonames <- deaths_1969_2020 %>% 
  ...
  distinct() %>% 
  mutate(temp = str_replace(kanton_bezirk_gemeinde, 
                            fixed("......"), "")) %>% 
...
deaths_1969_2020 %<>% 
  ...
  rename(year = jahr, 
         ...
         age = altersklasse,
         sex = geschlecht) %>% 
  mutate(sex = fct_recode(sex, Men = "Mann", Women = "Frau"),
         nationality = fct_recode(nationality, Swiss = "Schweiz", Foreigner = "Ausland"),
         age = fct_recode(age, 
                          "<20" = "Unter 20 Jahren", 
                          "20-29" = "20-29 Jahre", 
                          "30-39" = "30-39 Jahre", 

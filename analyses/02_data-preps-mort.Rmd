...
Needless to say that creates large amount of zeroe cells:
...
```{r echo=FALSE}
deaths_2015_2021_exp %>% 
  filter(GMDNAME %in% c("Zürich", "Bern", "Meienried")) %>% 
...
As above, Bern, Zurich & Seeland:   

```{r echo=FALSE}
deaths_2015_2021_bez %>% 
  filter(BZNR %in% c(112, 246, 243)) %>% 
  group_by(BZNR, date) %>% 
  summarise(deaths = sum(deaths)) %>% 
  ungroup() %>% 

...
## Predicting mid-year (and 2021) pops
...
For each GMDNR, age, sex, nationality stratum linear model is fit, then prediction is made for mid year.  

```{r}
population_mid <- population %>% 
  # filter(GMDNR == 261 | GMDNR == 6172) %>%
  ...
  do(lm(population ~ year, data = .) %>% 
       predict(., tibble(year = seq(2014.5, 2020.5, 1))) %>%
       tibble(year = seq(2014.5, 2020.5, 1), 
              population_mid = .)
  ) %>% 
  ungroup() %>% 
  mutate(year = as.integer(year + 0.5),
...
Predictions can be made also excluding data from year 2020 that was already affected by pandemic:  

```{r}
population_mid_no20 <- population %>% 
  # filter(GMDNR == 261 | GMDNR == 6172) %>%
  filter(year < 2020) %>%
  group_by(GMDNR, age, sex, nationality) %>% 
  do(lm(population ~ year, data = .) %>% 
  ...
  ungroup() %>% 
  mutate(year = as.integer(year + 0.5),
         population_mid_no20 = round(population_mid_no20)) %>% 
  arrange(year, GMDNR, sex, nationality, age)
```

```{r include=FALSE}
write_rds(population_mid_no20, "data/BfS/population_mid_no20.Rds")
```
...
For small municipality we might end up with sth funny, including data below 0?    

```{r echo=FALSE, warning=FALSE}
population %>% 
  filter(year < 2021) %>% 
  filter(GMDNR == 782 & age == "40-49") %>% 
  bind_rows(population_mid %>% 
              filter(GMDNR == 782 & age == "40-49") %>% 
              mutate(year = year - 0.5)) %>% 
  ggplot(aes(x = year)) + 
...
Predicting without 2020, a bit more wiggly:  

```{r echo=FALSE, warning=FALSE}
population %>% 
  filter(year < 2021) %>% 
  filter(GMDNR == 261 & age == "80+") %>% 
  bind_rows(population_mid_no20 %>% 
              filter(GMDNR == 261 & age == "80+") %>% 
              mutate(year = year - 0.5)) %>% 
  ggplot(aes(x = year)) + 
  geom_point(aes(y = population)) +
  geom_point(aes(y = population_mid_no20), color = "red") +
  facet_wrap(nationality~sex, scales = "free")
```

Differences of two approaches (lowest / highest 100 stratatata):  

```{r echo=FALSE}
population_mid %>% 
  left_join(population_mid_no20) %>% 
  mutate(diff = (population_mid_no20 - population_mid) / population_mid) %>% 
  arrange(diff) %>% 
  slice(1:100) %>% 
  datatable()
```
...
w_deaths_2015_2021_pop <- w_deaths_2015_2021_exp %>% 
  left_join(
...
rm(missing1, missing2); gc()
```

```{r include=FALSE}
rm(w_deaths_2015_2021_surplus)
rm(w_deaths_2015_2021_exp)
gc()
```
...
w_deaths_2015_2021_ar_pop <- w_deaths_2015_2021_ar %>% 
...
r19_bu <- r19_bu_orig %>% 
  # filter(between(r19_gklas, 1110, 1122)) %>% 
  # filter(!is.na(r19_gklas)) %>% 

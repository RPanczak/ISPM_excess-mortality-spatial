...
For each age, sex, nationality stratum, for each municipality separately, linear & Poisson models were fitted using the 2014-2019 data; then prediction was made for 2020 & 2021 years. The gist of this operation was to exclude the effect of pandemic from the population counts in years affected.   
...
### Results in large municipality {.tabset}
...
### Results in small municipality {.tabset}
...
**Note lack of prediction for oldest males!**  

## Getting mid-year pops {.tabset}
...
data <- population %>% 
  filter(year < 2020) %>% 
  bind_rows(population_ext_lm) %>% 
  # 2020 replaced by estimates from lm extrapol
...
for (i in 2014:2020){
  ...
  result <- data %>% 
    ...
    # group_by(GMDNR, age, sex, nationality) %>% 
    group_by(GMDNR, age, sex) %>% 
    summarise(population_mid_lm = mean(population)) %>% 
    ungroup() %>% 
    mutate(year = as.integer(i + 1),
           population_mid_lm = as.integer(round(population_mid_lm))) %>% 
    # arrange(year, GMDNR, sex, nationality, age)
    arrange(year, GMDNR, sex, age)
  
  if (i == 2014) {
    population_mid_lm <- result
  } else {
    population_mid_lm <- bind_rows(population_mid_lm, result)
  }
...
population %>% 
  ...
  bind_rows(population_mid_lm %>% 
              filter(GMDNR == 261 & age == "80+") %>% 
              mutate(year = year - .5)) %>% 
  ggplot(aes(x = year)) + 
  geom_point(aes(y = population)) +
  geom_point(aes(y = population_mid_lm), color = "red") +
...
w_deaths_2015_2021_pop <- read_rds("data/BfS-closed/w_deaths_2015_2021_exp.Rds") %>% 
  ...
  left_join(population_mid_lm) %>% 
  replace_na(list(population_mid_lm = 0)) %>% 
  left_join(population_mid_poi) %>% 
  replace_na(list(population_mid_poi = 0))
```
...
population_ar <- population %>% 
  ...
  group_by(year, ARNR, ARNAME, sex, age) %>% 
  summarise(population = sum(population)) %>% 
  ungroup()
...
st_ar_21 <- st_read("data/geo/st_ar_21.shp") %>% select(-ARNAME)
```
...
ar_21 <- st_read("data/geo/st_ar_21.shp") %>% 
  select(-ARNAME)
```

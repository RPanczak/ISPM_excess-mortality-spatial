...
Prepared population file is merged to deaths prepared in `02.Rmd`.  

Non-existent population count `communityXyearXageXsexXnationality` strata are assumed to mean 0 pops.  

```{r}
deaths_2015_2021_exp <- read_rds("data/BfS-closed/deaths_2015_2021_exp.Rds") 
...
There are some strata where count of deaths is larger than count of population. Usually the difference is in range of 1-2 and refers to very small counts, but community **Rennaz** in canton VD seems to have larger issue towards end of 2020?  

```{r}
deaths_2015_2021_surplus <- deaths_2015_2021_exp %>% 
  filter(deaths > population) %>% 
  arrange(GMDNR, date)
```

```{r echo=FALSE}
datatable(deaths_2015_2021_surplus)
```

```{r include=FALSE}
# exported to share with Cordula
deaths_2015_2021_surplus %>% 
  openxlsx::write.xlsx("data/pop_deaths_errors.xlsx", overwrite = TRUE,
                       creator = "@RPanczak", sheetName = "deaths",
                       firstRow = TRUE, colWidths ="auto")
```

```{r eval=FALSE, include=FALSE}
# Rennaz 
munch::swc_get_mun_history(5412)
munch::swc_get_mutations(5412)
```

```{r eval=FALSE, include=FALSE}
# missing checks
missing1 <- deaths_2015_2021_exp %>% 
  anti_join(population) %>% 
  select(GMDNAME) %>% 
  distinct()

missing2 <- population %>% 
  anti_join(deaths_2015_2021_exp) %>% 
  select(GMDNAME) %>% 
  distinct()
```

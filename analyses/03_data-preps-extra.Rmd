...
```{r eval=FALSE, include=FALSE}
summary(population_ext_lm$population_ext_lm)
```
...
summary(w_deaths_2015_2021_pop$population_mid_lm)
...
population_ar <- population_mid_poi %>% 
  left_join(raum) %>% 
  select(-GMDNR) %>% 
  # group_by(year, ARNR, sex, nationality, age) %>% 
  group_by(year, ARNR, age, sex) %>% 
  dplyr::summarise(population = sum(population_mid_poi)) %>% 
  ungroup()

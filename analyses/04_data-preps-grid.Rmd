---
title: "Spatial analyses of 2020-21 excess mortality in CH"
subtitle: "Grid & grided data preparation"
author: "Radek Panczak & Garyfallos Konstantinoudis"
date: "`r Sys.Date()`"
output:
  rmdformats::robobook:
    code_folding: show
    self_contained: true
    highlight: pygments
editor_options: 
  chunk_output_type: console
knit: (function(inputFile, encoding) {
  rmarkdown::render(inputFile, encoding = encoding, output_dir = "../docs") })
---

<!-- ----------------------------------------------------- -->

```{r setup, include = FALSE}
set.seed(12345)
options(scipen = 999)
options(max.print = "75")

library(pacman)
p_load(tidyverse, magrittr, scales,  
       DT, 
       sf, tmap)

tmap_mode("view")

import::from("sjmisc", "frq")
```

```{r conflicts, include = FALSE}
tidyverse::tidyverse_conflicts()

conflicted::conflict_scout()
```

```{r knit-setup, include = FALSE}
knitr::opts_knit$set(root.dir = rprojroot::find_rstudio_root_file())

knitr::opts_chunk$set(cache = FALSE,
                      prompt = FALSE,
                      tidy = FALSE,
                      comment = NA,
                      message = FALSE,
                      warning = FALSE)

knitr::opts_knit$set(width = 75)

mu <- Hmisc::markupSpecs$html
```

<!-- ----------------------------------------------------- -->

# Grid

## Municipality boundaries

Used to 'clip' the data to the country outline and introduce the boundaries within grid cells belonging to two different municipalities.  

```{r}
# gg21 <- read_rds("data/BfS/gg21.Rds")

st_gg21 <- read_rds("data/swisstopo/st_gg21.Rds") %>% 
  st_transform(2056)
```

## Offset

Open STATPOP dataset is used to define `offset` (ie. starting point; lowest left corner) for `st_make_grid`.  

```{r}
statpop_20 <- read_delim("data-raw/BfS/ag-b-00.03-vz2020statpop/STATPOP2020.zip", 
                         delim = ";", escape_double = FALSE, trim_ws = TRUE)[] %>% 
  mutate_all(as.integer)

offset <- statpop_20 %>% 
  summarise(minX = min(E_KOORD),
            minY = min(N_KOORD))

# statpop_20 %>% slice(1:10) %>% View()
rm(statpop_20); gc()
```

## Grid 

1-km grid, with lower left corner defined using `STATPOP` cells.

Grid is then overlaid with **swisstopo municipality data** to select only cells that overlay it.  

Lastly, **experimental** grid integrating municipality boundaries is also created.  

Also, adding ID for each cell (based on row number) and area for each of the iterations.  

```{r}
st_grid_1km <- st_gg21 %>% 
  st_make_grid(cellsize = 1000, square = TRUE, 
               offset = c(offset$minX, offset$minY)) %>% 
  st_sf() %>%
  st_cast("POLYGON") %>% 
  mutate(ID1 = row_number()) %>% 
  relocate(ID1) %>% 
  st_transform(2056)

st_grid_1km_over <- st_grid_1km %>% 
  st_filter(st_gg21, join = st_covers)  %>%
  mutate(ID2 = row_number()) %>% 
  relocate(ID1, ID2)

st_grid_1km_crop <- st_grid_1km_over %>% 
  st_intersection(st_gg21)  %>%
  mutate(ID3 = row_number(),
         area = st_area(.)) %>% 
  relocate(ID1, ID2, ID3, area)
```

```{r include=FALSE}
st_write(st_grid_1km, "data/grid/grid_1km.gpkg", 
         layer = "st_grid_1km", delete_dsn = TRUE)
st_write(st_grid_1km_over, "data/grid/grid_1km.gpkg", 
         layer = "st_grid_1km_over", append = TRUE)
st_write(st_grid_1km_crop, "data/grid/grid_1km.gpkg", 
         layer = "st_grid_1km_crop", append = TRUE)

# write_rds(st_grid_1km, "data/grid/st_grid_1km.Rds")
write_rds(st_grid_1km_over, "data/grid/st_grid_1km_over.Rds")
# write_rds(st_grid_1km_crop, "data/grid/st_grid_1km_crop.Rds")
```

Example of municipalities

### Bern   

```{r echo=FALSE}
st_join(st_grid_1km_over,
        st_gg21 %>% filter(GMDNR == 351),
        left = FALSE) %>% 
  st_geometry() %>% 
  qtm(fill = NULL)
```

### Scuol 

```{r echo=FALSE}
st_join(st_grid_1km_over,
        st_gg21 %>% filter(GMDNR == 3762),
        left = FALSE) %>% 
  st_geometry() %>% 
  qtm(fill = NULL)
```

Largest in terms of area.  

### Gottlieben

```{r echo=FALSE}
st_join(st_grid_1km_over,
        st_gg21 %>% filter(GMDNR == 4651),
        left = FALSE) %>% 
  st_geometry() %>% 
  qtm(fill = NULL)
```

Smallest in terms of area.  

## Grid comparison

Example of different precision of boundaries:  

```{r echo=FALSE}
map_centre <- st_sfc(st_point(c(2570200, 1203500)), crs = 2056) %>% 
  st_buffer(1000) %>% 
  st_as_sf()

grid <- st_filter(grid_1km_crop, map_centre)
st_grid <- st_filter(st_grid_1km_crop, map_centre)

tm_shape(grid) +
  tm_borders(col = "blue", lwd = 1, alpha = 0.5) +
  tm_shape(st_grid) +
  tm_borders(col = "red", lwd = 2, alpha = 0.5) 
```

## Small area cells

There are few examples of grid cells with very small area. These could be excluded if no population was found there?

```{r echo=FALSE}
st_grid_1km_crop %>% 
  st_join(st_grid_1km_crop %>% 
            arrange(area) %>% 
            slice(2),
          join = st_touches, left = FALSE) %>% 
  st_geometry() %>% 
  qtm()
```
<!-- ----------------------------------------------------- -->

# Gridded data

## Deaths

## Population

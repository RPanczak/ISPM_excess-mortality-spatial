...
```{r eval=FALSE}
# aggregating personal file to building level; into long format
r19_pe <- read_dta("data/STATPOP/r19_pe.dta") %>% 
...
```{r eval=FALSE, include=FALSE}
write_rds(r19_pe, "data/STATPOP/r19_pe.Rds")
```

```{r include=FALSE}
r19_pe <- read_rds("data/STATPOP/r19_pe.Rds")
```

```{r eval=FALSE, include=FALSE}
# !takes some time due to spatial operations!

r19_pe_geo <- read_dta("data/STATPOP/r19_pe.Rds") %>% 
  st_as_sf(coords = c("ecoord", "ncoord"), 
           crs = 2056,
           remove = TRUE)

st_write(r19_pe_geo, "data/STATPOP/geo.gpkg", 
         layer = "r19_pe_geo")

write_rds(r19_pe_geo, "data/STATPOP/r19_pe_geo.Rds")
...
# join to gem
# r19_pe_gem_link <- st_join(r19_pe_geo, st_gg21, join = st_nearest_feature)
...
# join to grid
# st_intersects much faster than st_nearest_feature
r19_pe_grid <- st_join(r19_pe_geo, grid_1km_over, join = st_intersects)

# r19_pe_grid %>% filter(is.na(GMDNR))

r19_pe_grid %<>% relocate(geometry, .after = last_col())

# View(st_drop_geometry(r19_pe_grid))
# frq(r19_pe_grid$KTNAME)

write_rds(r19_pe_grid, "data/STATPOP/r19_bu_link.Rds")

st_write(r19_pe_grid, "data/STATPOP/geo.gpkg", 
         layer = "r19_pe_grid", append = TRUE)
```

```{r include=FALSE}
r19_pe_geo <- read_rds("data/STATPOP/r19_pe_geo.Rds")
```

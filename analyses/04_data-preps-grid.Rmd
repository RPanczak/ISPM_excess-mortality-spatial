...
```{r include=FALSE}
rm(grid_1km_crop, grid_1km_crop_cent)
gc()
```
...
```{r}
m2 = lme4::glmer(pop ~ age + sex + year_month + (1 | ID), 
                 data = st_grid_pop_pred, 
                 family = poisson(link = "log"))

st_grid_pop_pred$m2 = as.integer(predict(m2, st_grid_pop_pred, type = "response"))
```
...
```{r}
m3 = lme4::glmer(pop ~ age + sex + year_month + (year_month | ID), 
                 data = st_grid_pop_pred, 
                 family = poisson(link = "log"))

st_grid_pop_pred$m3 = as.integer(predict(m3, st_grid_pop_pred, type = "response"))
```

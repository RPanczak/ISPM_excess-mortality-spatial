...
```{r}
ar21_wm_q <- read_rds("data/nb/ar21_wm_q.Rds")
```
...
m1_space <- 
  ar21 %>% 
  # tg3o_ar %>% 
  left_join(m1$summary.random$ARNR, by = c("ARNR" = "ID")) %>% 
...
m2_space <- 
  ar21 %>% 
  # tg3o_ar %>% 
  left_join(m2$summary.random$ARNR, by = c("ARNR" = "ID")) %>% 
  mutate(`0.5quant` = exp(`0.5quant`))
...
m3_space <- 
  ar21 %>% 
  # tg3o_ar %>% 
  left_join(m3$summary.random$ARNR, by = c("ARNR" = "ID")) %>% 
  mutate(`0.5quant` = exp(`0.5quant`))
